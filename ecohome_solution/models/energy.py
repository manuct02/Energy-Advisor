"""
Los modelos de datos de energía para EcoHome Energy Advisor
"""
...
class DatabaseManager:
    ...
    def create_tables(self):
        """Aquí es donde se crean las tablas"""
    ...
    def add_usage_record(self, timestamp: datetime, consumption_kwh: float, device_type: str= None, device_name: str= None, cost_usd: float= None):
        """ Aquí se guarda el uso de la energía """
        session= self.get_session()
        try:
            record= EnergyUsage(timestamp= timestamp, consumption_kwh= consumption_kwh, device_type=device_type, device_name= device_name, cost_usd=cost_usd)
            session.add(record)
            session.commit()
            return record
        finally:
    ...
    def add_generation_record(self, timestamp: datetime, generation_kwh: float, weather_condition: str= None, temperature_c: float= None, solar_irradiance: float= None):
        """ Añade el registro de la generación solar """
        session= self.get_session()
        try:
    ...
    def get_usage_by_date_range(self, start_date: datetime, end_date: datetime):
        ...
        session= self.get_session()
        try:
            return session.query(EnergyUsage).filter(EnergyUsage.timestamp>= start_date, EnergyUsage.timestamp<= end_date).order_by(EnergyUsage.timestamp).all()
        finally:
            session.close()
